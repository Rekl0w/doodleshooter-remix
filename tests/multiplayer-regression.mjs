...
const die=async(p,fast=true)=>{await free(p);await p.evaluate(fast=>{const g=__game;g.player.shieldT=0;g.player.takeDamage(1000);if(fast)g.game.respawnAt=performance.now()+150;},fast);await p.waitForFunction(()=>__game.game.state==='dying');};
const ready=p=>p.waitForFunction(()=>{const b=document.querySelector('#respawnBtn');return b&&!b.disabled&&!b.parentElement.hidden;});
const place=async(p,x,z)=>p.evaluate(({x,z})=>{const g=__game;g.player.body.pos.set(x,0,z);g.player.body.vel.set(0,0,0);g.player.yaw=0;g.player.pitch=0;g.player.shieldT=0;g.input.usingGamepad=true;g.game.menu=false;g.hud.hideScreen();}, {x,z});
try {
 const host=await open('Host');await host.locator('input[value="private"]').check();await host.locator('#createBtn').click();await host.waitForFunction(()=>__game.net.active);const code=await host.evaluate(()=>__game.net.code);
 ...
 await guest.evaluate(()=>{const n=__game.net;window.damageHandler=n.handlers.get('pdmg');n.on('pdmg',(d,f)=>setTimeout(()=>damageHandler(d,f),350));});
 ...
 await guest.evaluate(()=>__game.net.on('pdmg',damageHandler));
 await guest.evaluate(()=>__game.player.shieldT=5);const protectedHp=await guest.evaluate(()=>__game.player.hp);
 ...
 await guest.evaluate(()=>__game.player.shieldT=0);
 ...
 // Replaying the same packet cannot damage twice, even with a forged by field.
 const packet=await rival.evaluate(id=>{const g=__game,t=g.remote.get(id);return {id:'duplicate-test',life:t.lifeId,amount:7,src:'rifle',by:'forged'};},guestId);
 const before=await guest.evaluate(()=>__game.player.hp);await rival.evaluate(({id,d})=>{__game.net.sendTo(id,'pdmg',d);__game.net.sendTo(id,'pdmg',d);},{id:guestId,d:packet});await guest.waitForFunction(hp=>__game.player.hp<hp,before);
 check('duplicate packet applies damage once and uses real sender',await guest.evaluate(({hp,id})=>__game.player.hp===hp-7&&__game.player.lastHitBy===id,{hp:before,id:rivalId}));
 ...
 const newHp=await guest.evaluate(()=>__game.player.hp);await guest.evaluate(()=>__game.player.shieldT=0);
 ...
 await host.evaluate(id=>{const g=__game;g.scores.get(id).kills=19;g.player.lastHitBy=id;g.player.lastHit={amount:150,src:'sniper'};g.player.takeDamage(1000);},guestId);
